 ...
 * the `+`, `*`, and concatenation (`||`) operators. For example, for a number
 * list of `[2, 31]`, this will return an array of the values 62, 33, and 231.
 * Operations are always applied in left to right order, so mathematical order
 * of operations rules such as PEMDAS or BODMAS are ignored.
 *
 * @param numberList    The list of numbers to compute.
 * @returns {number[]}  The results of all possible combinations of adding,
 *                      multiplying, and concatenating the provided numbers.
 */
function findCalculation(numberList) {
    if (numberList.length === 2) {
        let value1 = numberList[0] * numberList[1]
        let value2 = numberList[0] + numberList[1]
        let value3 = parseInt("" + numberList[0] + numberList[1])
...
function solve(input) {
    ...
    for (const equation of equationList) {
     ...
     console.log("There are %d equations that can be solved with +, *, and || operators.", totalFixed)
}
